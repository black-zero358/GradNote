...
- **认证授权**: python-jose 3.4.0, passlib 1.7.4
- **LLM集成**: 支持OpenAI兼容接口的模型（如火山引擎、智谊等）
  - 默认LLM模型: deepseek-v3-250324
  - 默认VLM模型: doubao-1-5-vision-pro-32k-250115
  - 默认Embedding模型: doubao-embedding-large-text-240915
- **框架**:
  - LangChain 0.3.23: 用于管理LLM/VLM调用和知识检索
  - LangGraph: 用于构建复杂工作流（如解题流程）
  - Langfuse 2.60.2: 用于可观察性和性能分析

### 2.2 数据存储

- **关系型数据库**: PostgreSQL（使用psycopg2驱动）
- **向量数据库**: PostgreSQL+pgvector（存储知识点向量嵌入）
- **缓存**: Redis 5.0.1（用于缓存频繁访问的数据）
- **文件存储**: 本地文件系统（用于存储错题图片）
...
```
POST /api/v1/questions - 创建新错题
GET /api/v1/questions - 获取错题列表
GET /api/v1/questions/{question_id} - 获取错题详情
PUT /api/v1/questions/{question_id} - 更新错题
DELETE /api/v1/questions/{question_id} - 删除错题
POST /api/v1/questions/from-image - 从图片创建错题（一步完成图片处理与错题创建）
```

### 7.3 图像处理API

```
POST /api/v1/image/process - 处理错题图像并提取文本（不创建错题，仅处理图像）
POST /api/v1/image/extract-answer - 从图片中提取答案部分
```

### 7.4 知识点管理API

```
GET /api/v1/knowledge/structure - 基于结构化信息（科目、章节、小节）查询知识点
GET /api/v1/knowledge/search - 按条件搜索知识点
GET /api/v1/knowledge/popular - 获取最热门的知识点（根据标记次数）
GET /api/v1/knowledge/subjects - 获取所有科目列表
GET /api/v1/knowledge/chapters - 获取指定科目的所有章节
GET /api/v1/knowledge/sections - 获取指定科目和章节的所有小节
GET /api/v1/knowledge/{knowledge_point_id} - 根据ID获取知识点详情
POST /api/v1/knowledge/mark/{knowledge_point_id} - 增加知识点标记次数
POST /api/v1/knowledge/user-mark - 创建用户知识点标记记录
GET /api/v1/knowledge/user-marks - 获取当前用户的所有标记
POST /api/v1/knowledge/analyze-from-question - 分析题目文本，返回可能的知识点类别
POST /api/v1/knowledge/extract-from-solution - 从解题过程中提取使用的知识点
```

### 7.5 解题API

```
POST /api/v1/solving/{question_id} - 解答错题（使用指定的知识点）
```
